...
public class RButton : MonoBehaviour
{
    ...
    void Start()
    {
        this.ValveS = GameObject.Find("ValveSound");
    ...
    void Update()
    {
        if (!Locked)
        {
            if (Input.GetKeyDown(KeyCode.R) && !isValve && DoorObject != null && DoorObject.Remote && NearView()) // 1.lever and 2.button
            {
            ...
            else if (isValve && RampObject != null) // 3.valve
            {
                ...
                if (Input.GetKey(KeyCode.R) && NearView())
                {
                    if (Input.GetKeyDown(KeyCode.R))
                    {
                        this.ValveS.GetComponent<AudioSource>().Play();
                    }
                    if (valveBool)
                    {
    ...
    void OnMouseEnter()
    {
        ...
        pickUpText.text = "레버를 돌리려면 " + "<color=yellow>" + "(Right)" + "</color>";
