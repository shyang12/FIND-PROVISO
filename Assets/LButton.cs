...
public class LButton : MonoBehaviour
{
    ...
    void Update()
    {
        if (!Locked)
        {
            if (Input.GetKeyDown(KeyCode.O) && !isValve && DoorObject != null && DoorObject.Remote && NearView()) // 1.lever and 2.button
            {
            ...
            {    
                ...
                if (Input.GetKey(KeyCode.O) && NearView())
                {
    ...
    void OnMouseEnter()
    {
        ...
        pickUpText.text = "레버를 돌리려면 " + "<color=yellow>" + "(Left)" + "</color>";
    }
